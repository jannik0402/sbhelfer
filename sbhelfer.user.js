...
(function() {
	...
	if(metas[0].attributes[1].nodeValue.toString().indexOf("noindex") > -1){
		alert("NOINDEX!");
		document.getElementById("main-nav").innerHTML += '<div style="position:absolute;top:10px;right:10px;background-color:red;color:white;padding:3px;">NOINDEX</div>';
	}else{
		document.getElementById("main-nav").innerHTML += '<div style="position:absolute;top:10px;right:10px;background-color:green;color:white;padding:3px;">INDEX</div>';
	}
	...
	if(evt.keyCode == '51'){
		...
		if(neuerlink.indexOf("?byPassDigCmsCache") > -1){
        	...
        	if(neuerlink != "/"){
			window.open('https://keen-albattani.85-215-230-13.plesk.page/generatoren/ausgabe/design2/tarue.php?link=' + neuerlink);
